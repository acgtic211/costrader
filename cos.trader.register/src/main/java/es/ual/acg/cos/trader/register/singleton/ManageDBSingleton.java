...
public final class ManageDBSingleton {
	
	private static ManageDBSingleton INSTANCE;
	
	private HbDataStore dataStore;
	private HbDataStore dataStoreAC;
	private HbDataStore dataStoreCC;
	private boolean dataStoreOn = false;
	private boolean dataStoreACOn = false;
	private boolean dataStoreCCOn = true;

	private static final Logger LOGGER = Logger.getLogger(ManageDBSingleton.class);
	
	private ManageDBSingleton() {
	...
	public synchronized static ManageDBSingleton getInstance() {
	    if (INSTANCE == null) {
	        INSTANCE = new ManageDBSingleton();
	    }
	    return INSTANCE;
	}
    ...
    hibernateProperties.setProperty(Environment.DIALECT, org.hibernate.dialect.MySQL5InnoDBDialect.class.getName());
		...
		final String dataStoreName = "traderccbig";
