...
public class ManageAbstractComponentsSingleton {

	private static ManageAbstractComponentsSingleton INSTANCE;

	private HbDataStore dataStoreAC;

	private static final Logger LOGGER = Logger.getLogger(ManageAbstractComponentsSingleton.class);

	public ManageAbstractComponentsSingleton() {
		LOGGER.info("ManageAbstractComponentsSingleton created");
	}

	public synchronized static ManageAbstractComponentsSingleton getInstance() {
		if (INSTANCE == null) {
			INSTANCE = new ManageAbstractComponentsSingleton();
		}
		return INSTANCE;
	}
	 ...
	 * LOGGER.info("ManageAbstractComponents component is ready"); }
	 */
	...
	public AbstractComponentInfo getACInfoFromID(String acID) {
		...
		// Create query
		Query query = session
				.createQuery("FROM AbstractComponentSpecification acs WHERE acs.componentID = '" + acID + "'");
		List<?> acsList = query.list();
		if (acsList.size() != 0) {
